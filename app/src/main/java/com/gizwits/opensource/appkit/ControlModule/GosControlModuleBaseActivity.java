...
public class GosControlModuleBaseActivity extends GosBaseActivity {
	 ...
	 */
	// 数据点"legacy"对应的标识名
	protected static final String KEY_LEGACY = "legacy";
	// 数据点"LED1"对应的标识名
	...
	// 数据点"Moter"对应的标识名
	protected static final String KEY_MOTER = "Moter";
	// 数据点"LED_G"对应的标识名
	 ...
	 * ===========================================================
	 */
	// 数据点"Moter"对应seekbar滚动条补偿值
	protected static final int MOTER_OFFSET = 0;
	// 数据点"Moter"对应数据点增量值
	protected static final int MOTER_ADDITION = -5;
	// 数据点"Moter"对应数据点定义的分辨率
	protected static final int MOTER_RATIO = 1;
	...
	// 数据点"LED1"对应的存储数据
	protected static boolean LED1;
	// 数据点"LED2"对应的存储数据
	protected static boolean LED2;
	// 数据点"LED3"对应的存储数据
	protected static boolean LED3;
	// 数据点"LED4"对应的存储数据
	protected static boolean LED4;
	// 数据点"Moter"对应的存储数据
	protected static int Moter;
	// 数据点"LED_G"对应的存储数据
	...
	protected void getDataFromReceiveDataMap(ConcurrentHashMap<String, Object> dataMap) {
		...
		if (dataMap.get("data") != null) {
			...
			for (String dataKey : map.keySet()) {
				if (dataKey.equals(KEY_LEGACY)) {
					legacy = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_LED1)) {
					LED1 = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_LED2)) {
					LED2 = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_LED3)) {
					LED3 = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_LED4)) {
					LED4 = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_MOTER)) {
			
					Moter = (Integer) map.get(dataKey);
				}
